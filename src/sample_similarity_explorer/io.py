...
def load_feature_matrix(path: str) -> pd.DataFrame:
    ...
    if df.index.duplicated().any():
        ...
        raise ValueError(f"Feature identifiers must be unique. Duplicates found: {duplicated_features[:5]}")
    ...
    if df.columns.duplicated().any():
        ...
        raise ValueError(f"Sample names must be unique. Duplicates found: {duplicated_samples[:5]}")
